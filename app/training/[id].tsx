...
import { getTrainingMenuById } from "@/lib/training-data";

export default function TrainingDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<"overview" | "procedure" | "tips">("overview");
  const menu = id ? getTrainingMenuById(id) : null;
    ...
    <ScreenContainer className="p-4">
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-surface rounded-lg p-4 mb-4 border border-border">
          ...
          <Text className="text-sm text-muted">練習タイプ: {menu.trainingType}</Text>
        </View>
        ...
        {activeTab === "procedure" && (
          <View className="gap-4 pb-6">
            <Text className="text-sm font-semibold text-foreground">練習の手順</Text>
        ...
        {activeTab === "tips" && (
          <View className="gap-4 pb-6">
            <Text className="text-sm font-semibold text-foreground">練習のコツ</Text>
