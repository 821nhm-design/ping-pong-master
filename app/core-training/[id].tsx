...
export default function CoreTrainingDetailScreen() {
  ...
  const menu = id ? coreTrainingsMega.find((m) => m.id === id) : null;
    ...
    <ScreenContainer className="p-4">
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-surface rounded-lg p-4 mb-4 border border-border">
          ...
          {(menu as any).sets && <Text className="text-sm text-muted mb-1">セット数: {(menu as any).sets}</Text>}
          {(menu as any).reps && <Text className="text-sm text-muted">回数: {(menu as any).reps}</Text>}
        </View>
